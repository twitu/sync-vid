...
use futures::{select, FutureExt, channel::oneshot, task::{LocalSpawn, LocalSpawnExt}, executor::LocalPool};
...
impl NetworkInterface for Client {
    ...
    fn join_room(room_id: &String) -> Self {
        ...
        local_pool.spawner().spawn_local(async {
            loop_fut.await;
    ...
    fn get_next_event(&mut self) -> Option<SyncEvent> {
        ...
        self.socket.receive().iter().map(|(peer, data)| {
                web_sys::console::log_1(&format!("Received Data Results: {:?} from {}", &data, &peer).into());
                (peer, data)
            }).last().map(|(peer, data)| {
                (peer, match std::str::from_utf8(&data).unwrap() {
                    "play" => SyncEvent::Play,
