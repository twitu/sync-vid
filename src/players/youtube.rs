...
impl PlayerInterface for YouTube {
    fn get_video(&self) -> Option<&web_sys::HtmlVideoElement> {
        self.video.as_ref()
    }
    ...
    fn initialize(&mut self) -> Result<(), String> {
        web_sys::window()
                    ...
                    .and_then(|w| w.document().ok_or("Document not found for the window".to_owned()))
                    .and_then(|doc| 
                        doc.query_selector("video.html5-main-video")
                        .map_err(|er| er.as_string().unwrap_or("Can't search for videos".to_owned()))
                        .and_then(|res| res.ok_or("Couldn't find any videos".to_owned()))
                    )
                    .map(|elem| {
                        self.video = Some(HtmlVideoElement::from(JsValue::from(elem)));
                        ()
                    })
    }
}